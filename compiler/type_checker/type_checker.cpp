    ...
    namespace compiler {
            ...
            for (int i = 0; i < stmt->vars.size(); i++) {
                ...
                if (stmt->initValues[i] != nullptr) {
                    ...
                    if (!stmt->types[i]->isEqual(stmt->initValues[i]->objectType)) {
                        TypeError::typeNotMatch(stmt->types[i], stmt->initValues[i]->objectType,
                                                stmt->initValues[i]->line, stmt->initValues[i]->column);
                    }
                }
                program->varTable->set(((IdentifierExpr *) stmt->vars[i])->identifier, stmt->types[i],
                                       stmt->vars[i]->line, stmt->vars[i]->column);
            }
        }
        ...
        void TypeChecker::checkFuncDef(FuncDefStmt *stmt) {
            ...
            for (int i = 0; i < stmt->parameters.size(); i++) {
                ...
                if (stmt->parameterTypes[i]->singleObjectType() == SingleObjectType::Void) {
                    TypeError::typeNotAllowed(stmt->parameterTypes[i], stmt->parameters[i]->line,
                                              stmt->parameters[i]->column);
                }
                funcVarTable->set(((IdentifierExpr *) stmt->parameters[i])->identifier, stmt->parameterTypes[i],
                                  stmt->parameters[i]->line, stmt->parameters[i]->column);
            }
        ...
        void TypeChecker::checkArrayLiteralExpr(ArrayLiteralExpr *expr, VarTable *varTable) {
            for (int i = 0; i < expr->exprs.size(); i++) {
                ...
                if (!expr->exprs[i]->objectType->isEqual(expr->exprs[0]->objectType)) {
                    TypeError::typeNotMatch(expr->exprs[0]->objectType, expr->exprs[i]->objectType,
                                            expr->exprs[i]->line, expr->exprs[i]->column);
                }
            }
        ...
        void TypeChecker::checkArrayIndexExpr(ArrayIndexExpr *expr, VarTable *varTable) {
            ...
            if (expr->index->objectType->singleObjectType() != SingleObjectType::Int) {
                TypeError::typeNotMatch(new IntObject(), expr->index->objectType, expr->index->line,
                                        expr->index->column);
            }
        ...
        void TypeChecker::checkTypeConversionExpr(TypeConversionExpr *expr, VarTable *varTable) {
            ...
            if (expr->targetType == SingleObjectType::Char &&
                expr->expr->objectType->singleObjectType() == SingleObjectType::Bool) {
                TypeError::typeNotAllowed(expr->expr->objectType, expr->expr->line, expr->expr->column);
            }
            if (expr->targetType == SingleObjectType::Bool &&
                expr->expr->objectType->singleObjectType() == SingleObjectType::Char) {
                TypeError::typeNotAllowed(expr->expr->objectType, expr->expr->line, expr->expr->column);
            }
            if (expr->targetType == SingleObjectType::String &&
                expr->expr->objectType->singleObjectType() == SingleObjectType::Char) {
                TypeError::typeNotAllowed(expr->expr->objectType, expr->expr->line, expr->expr->column);
            }
        ...
        void TypeChecker::checkFuncCallExpr(FuncCallExpr *expr, VarTable *varTable) {
            if (((IdentifierExpr *) expr->name)->identifier == "print") {
                ...
                FuncDefStmt *func = program->funcTable->getStmt(((IdentifierExpr *) expr->name)->identifier, expr->line,
                                                                expr->column);
        ...
        void TypeChecker::checkLocalVarDefStmt(VarDefStmt *stmt) {
            for (int i = 0; i < stmt->vars.size(); i++) {
                ...
                curBlock->varTable->set(((IdentifierExpr *) stmt->vars[i])->identifier, stmt->types[i],
                                        stmt->vars[i]->line, stmt->vars[i]->column);
            }
        }
        ...
        void TypeChecker::checkIfStmt(IfStmt *stmt) {
            for (int i = 0; i < stmt->conditions.size(); i++) {
                ...
                if (stmt->conditions[i]->objectType->singleObjectType() != SingleObjectType::Bool) {
                    TypeError::typeNotMatch(new BoolObject(), stmt->conditions[i]->objectType,
                                            stmt->conditions[i]->line, stmt->conditions[i]->column);
                }
                Block *temp = curBlock;
        ...
        void TypeChecker::checkBinaryExpr(BinaryExpr *expr, VarTable *varTable) {
            ...
            if (expr->op == OpType::Add) {
                if (expr->left->objectType->singleObjectType() == SingleObjectType::String ||
                    expr->left->objectType->singleObjectType() == SingleObjectType::Char) {
                    if (expr->right->objectType->singleObjectType() == SingleObjectType::String ||
                        expr->right->objectType->singleObjectType() == SingleObjectType::Char) {
                        if (expr->left->objectType->singleObjectType() == SingleObjectType::Char) {
                            expr->left = new TypeConversionExpr(SingleObjectType::String, expr->left, expr->left->line,
                                                                expr->left->column);
                            expr->left->objectType = new StringObject();
                        }
                        if (expr->right->objectType->singleObjectType() == SingleObjectType::Char) {
                            expr->right = new TypeConversionExpr(SingleObjectType::String, expr->right,
            ...
            if (!expr->left->objectType->isEqual(expr->right->objectType) ||
                expr->left->objectType->singleObjectType() == SingleObjectType::Array) {
                TypeError::invalidOp(expr->op, expr->line, expr->column);
            }
            switch (expr->op) {
                case OpType::Add:
